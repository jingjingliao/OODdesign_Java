...
public class ReservationService {
  ...
  private static void reserveSeats(String customerName, boolean needWheelChairSeat, int reserveNum, Theater theater) {
    ...
    if (bestRow == null)
      ...
      for (int i = 0; i < bestRow.getSeatNums() && count > 0; i++)
        if (bestRow.get(i).getReserveFor() == null){
          bestRow.get(i).setReserveFor(customerName);
          count -= 1;
        }
  ...
  private static boolean nomarlChairFull(List<Row> rows){
    for (int i = 0; i < rows.size(); i++){
      if (!rows.get(i).isWheelchairRow()){
        if (rows.get(i).get(i).getReserveFor() == null)
          return false;
      }
    }
    return true;
  ...
  private static Row getBestRowForWheelChair(List<Row> rows, int mid) {
    return rows.stream().filter(row -> row.isWheelchairRow() && isRowReservable(row, reserveNum)).min(
        Comparator.comparingInt(r -> Math.abs(r.getRowNum() - mid))).orElse(null);
  }
  ...
  private static boolean isRowReservable(Row row, int reserveNum){
    int count = 0;
    for (int i = 0; i < row.getSeatNums(); i++){
      if (row.get(i).getReserveFor() == null)
        count += 1;
    }
